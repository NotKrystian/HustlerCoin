...
interface ITREASURY20 is IERC20 {
    ...
    function bondHustlerETH(uint256 rate) external payable;
}
...
contract Pay0utStaking is Ownable {
    ...
    uint256 public ratePerHustler = 0;
    ...
    event Bond(address indexed user, uint256 amountHustler, uint256 amountTOKEN, uint256 rate);
    event FinishBond(address indexed user, uint256 amountHustler);
    event BondStakeHustler(address indexed bond, address indexed user, uint256 amount);

    function bondHustlerToken(uint256 deposit) public {  //deposit Hustler Token for staking and log timestamp of transaction using ETH
        if(ITREASURY20(treasuryAddress).mintingStatus()) {
            ...
            currentRate = ((1e4 - ((tokensBonded[address(this)].mul(1e4))/maxTokens)) * ratePerHustler);
            ITREASURY20(treasuryAddress).bondHustlerETH{value: deposit}(currentRate);
            ...
            userProfile[msg.sender].add(tokensInTranscation);
            userProfileTime[msg.sender] = block.timestamp;
            userEndTime[msg.sender] = block.timestamp + bondLength;
        }
        else {
    ...
    function endBond() public {
        if(block.timestamp > userEndTime[msg.sender]) {
            revert("Bond still has time till expiry");
        }
        else {
            require(IERC20(HustlerToken).transfer(msg.sender, userProfile[msg.sender]), "Failed to transfer tokens to contract");
    ...
    function setBondActive(bool active) public onlyOwner() {
        bondActive = active;
    }

    function setBondRate(uint256 rate) public onlyOwner() {
        ratePerHustler = rate;
    }

    function setBondMaxTokens(uint256 max) public onlyOwner() {
        maxTokens = max;
    }

    function setBondToken(address token) public onlyOwner() {
        bondToken = token;
    }
